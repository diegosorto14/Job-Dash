...
import remoteStatus from "./data/app_status.json";
import "./App.css";
...
function loadApps() {
  try {
    const local = JSON.parse(localStorage.getItem("frp_apps") || "{}");
    // Merge remote app_status (from Gmail parser) with local overrides.
    // Local manual changes win; remote fills in anything not set locally.
    const merged = {};
    for (const [id, remote] of Object.entries(remoteStatus)) {
      const localEntry = local[id] || {};
      merged[id] = {
        status:    localEntry.status    || remote.status,
        deadline:  localEntry.deadline  || "",
        notes:     localEntry.notes     || "",
        interview: remote.interview     || localEntry.interview || null,
      };
    }
    // Keep any local entries not in remoteStatus
...
const FORMAT_COLORS = {
  "Phone Screen":          { bg: "#EBF2FA", color: "#185FA5" },
  "Video Call":            { bg: "#F0F9FF", color: "#0369A1" },
  "HireVue (Recorded Video)": { bg: "#FDF4FF", color: "#7C3AED" },
  "In-Person":             { bg: "#F0FDF4", color: "#16A34A" },
  "Online Assessment":     { bg: "#FFF7ED", color: "#C2410C" },
  "Superday / Final Round":{ bg: "#FEF2F2", color: "#B91C1C" },
  "Interview":             { bg: "#F8FAFC", color: "#475569" },
};
...
export default function App() {
  ...
  const modalCompany = modal ? companies.find(c => c.id === modal) : null;
  const modalApp     = modal ? (apps[modal] || {}) : {};
  const interview    = modalApp.interview || null;
                      ...
                      <tr key={c.id}>
                        ...
                        <td>
                          <button className="note-btn" onClick={() => openModal(c.id)}>
                            {iv
                              ? <span className="interview-badge">{iv.format}</span>
                              : (app.notes ? "📝 Edit" : "+ Add")}
                          </button>
                        </td>
                        <td>
          ...
          <>
            ...
            {newJobs.length === 0 ? (
              <div className="empty-feed">
                <p>🔍 No new postings found yet.</p>
                <p className="muted">The daily scanner runs at 9 AM ET. Check back after programs start opening in July.</p>
              </div>
            ) : (
      ...
      {modal && (
        <div className="modal-overlay" onClick={() => setModal(null)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <h3>{modalCompany?.company}</h3>
            <p className="modal-program">{modalCompany?.program}</p>

            {/* Interview details block — auto-populated from Gmail */}
            {interview && (
              <div className="interview-block">
                <div className="interview-header">
                  <span
                    className="interview-format-tag"
                    style={FORMAT_COLORS[interview.format] || FORMAT_COLORS["Interview"]}
                  >
                    {interview.format}
                  </span>
                  {interview.date && (
                    <span className="interview-date-tag">
                      {interview.date}{interview.time ? ` · ${interview.time}` : ""}
                    </span>
                  )}
                </div>
                {interview.summary && (
            ...
            <textarea
              ...
              placeholder="Add notes, contacts, prep tips, round details…"
              rows={5}
            />
